...
class LayerGroup(object):
    """Group of layers wrapped up in a context"""

    OP_FUNCS = {
        'batchnorm': append_batchnorm,
        'dropout': append_dropout,
        'dense': append_dense,
        'conv': append_conv,
        'deconv': append_deconv,
        'maxpooling': append_maxpooling,
        'unpool': append_unpool,
        'unstride': append_unstride,
        'onehot_encode': append_onehot_encode,
        'onehot_decode': append_onehot_decode
    }

    def __init__(self, op_order, layer_group_config):

        self.op_order = op_order
        self.layer_group_config = layer_group_config

        if self.op_order[0] not in self.layer_group_config:
            raise KeyError(
                '{op} params not specified in {op} layer group'.format(op=self.op_order[0])
            )

        self.layer_stack = []

    def build(self, architecture, input_layer, name):
        """Stack a bunch of layer operations together in a group"""

        self.layer_stack.append(input_layer)

        with tf.name_scope(name):
            for op_type in self.op_order:
                if op_type in self.layer_group_config:

                    _func = self.OP_FUNCS.get(op_type, lambda x: x)
                    op_config = self.layer_group_config[op_type]

                    self.layer_stack.append(
                        _func(architecture, self.layer_stack[-1], op_config, op_type)
                    )
