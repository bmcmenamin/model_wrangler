...
def make_linear_reg_testdata(in_dim=4, n_samp=1000):
...
def make_linear_cls_testdata(in_dim=4, n_samp=1000):
    """Make sample data for linear regression
    """
    signal = zscore(np.random.randn(n_samp, 1), axis=0)
    X = zscore(np.random.randn(n_samp, in_dim), axis=0)
...
def compare_scikt_and_tf(sk_class, tf_class, X, y, sk_params={}):
    ...
    tf_model = tf_class(name='test', in_size=X.shape[1], num_epochs=50)
...
def test_logistic_regr(in_dim=4):
